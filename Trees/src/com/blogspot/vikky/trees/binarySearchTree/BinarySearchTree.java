...
public class BinarySearchTree<T extends Comparable<T>> {
     ...
     * nodes on the longest path between two leaves in the tree. The complexity
     * of this is O(n^2). A better solution of O(n) is to fetching the maxHeight
     * of tree using references. Since Java Integer is Immutable, this is not
     * possible. To implement this, we need a Integer like class which has a
     * 'setValue' function. (mutable). Refer online for a solution.
     */
    private int diameter(BSTNode node) {
        ...
        return (lh + rh + 1) > (dl > dr ? dl : dr) ? (lh + rh + 1) : (dl > dr
                ? dl
                : dr);
    }
    ...
    /*
     * Counting the number of unique Binary Search Trees that can be formed, for
     * a given N
     */
    ...
    /*
     * The other algorithm is to do inOrder Traversal and check if it is sorted
     * or not
     */
    private boolean isBST(BSTNode node, T max, T min) {
        ...
        if ((node.value.compareTo(min) < 0) || (node.value.compareTo(max) > 0))
            return false;
        return (isBST(node.left, node.value, min) && isBST(node.right, max,
                node.value));
    }

    /*
     * A redundant method just to test tamper the tree to verify isBST method.
     */
    ...
    /*
     * Inorder traversal on a BSTree prints out the list in a sorted order. This
     * is the principle of TreeSort. But if the tree is unbalanced, the worst
     * case is O(n^2). Explore self balancing trees for that.
     */
    ...
    /*
     * The other idea is to use parent field. This way we do not need to use a
     * stack
     */
    ...
    /*
     * PostOrder traversal is reverse of right to left preOrder traversal. Hence
     * perform a right to left preOrder traversal and use another stack to
     * reverse it
     */
    ...
    private BSTNode constructBinaryTreefromArray(ArrayList<BSTNode> arr,
    ...
    /*
     * Tree Rotation. Adding a new parameter called 'parent' since tree rotation
     * need information about the parent.
     */
   ...
   /* private boolean isBalanced(BSTNode node) {
    ...
    private boolean isBalanced(BSTNode node) {
        ...
        int htL = maxHeight(node.left);
        int htR = maxHeight(node.right);
